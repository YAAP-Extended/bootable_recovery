...
static bool CheckAbSpecificMetadata(const std::map<std::string, std::string>& metadata) {
  ...
  if (pkg_post_timestamp_string.empty() ||
    ...
    // Check if this is a userdebug or eng build
    const auto build_type = android::base::GetProperty("ro.build.type", "");
    if (build_type != "userdebug" && build_type != "eng") {
      LOG(ERROR) << "Update package is older than the current build, expected a build "
                    "newer than timestamp "
                 << build_timestamp << " but package has timestamp " << pkg_post_timestamp
                 << " and downgrade not allowed for " << build_type << " builds.";
      return false;
    }
    LOG(INFO) << "Downgrade allowed for " << build_type << " build";
  }
...
static InstallResult VerifyAndInstallPackage(Package* package, bool* wipe_cache,
  ...
  // Skip verification and proceed directly to installation
  ui->SetProgressType(RecoveryUI::DETERMINATE);
  ui->ShowProgress(1.0, VERIFICATION_PROGRESS_TIME);
...
bool verify_package([[maybe_unused]] Package* package, RecoveryUI* ui) {
  // Skip verification and return success
  ui->Print("Skipping package verification...\n");
  return true;
}
